...
async function insertFacilityRequirements() {
  ...
  for (let facilityId = 1; facilityId <= maxFacilities; facilityId++) {
    for (let documentId = 1; documentId <= maxDocuments; documentId++) {
      if (returnRandomBoolean()) continue;
...
async function insertDocumentWorkers() {
  ...
  for (let workerId = 1; workerId <= maxWorkers; workerId++) {
    for (let documentId = 1; documentId <= maxDocuments; documentId++) {
      ...
      documentWorkers.push({ worker_id: workerId, document_id: documentId });
    }
  }
